...
from Code.Q_learning.Trash_Attempt_2_DoubleQ.State import State


class QAgent:
    def __init__(self, env, discount_rate=0.995):
        self.env = env
    ...
    def create_Q_table(self):
        ...
        self.Qtable_1 = np.zeros((*bin_lengths, len(self.action_space)))
        self.Qtable_2 = np.zeros((*bin_lengths, len(self.action_space)))

    def train(self, simulations, learning_rate, epsilon=0.05,
              adaptive_epsilon=False, epsilon_start=1, epsilon_end=0.01, epsilon_decay=2000):
        np.random.seed(0)
        ...
        for i in range(simulations):
            ...
            while not done:
                if np.random.uniform() < epsilon:
                    ...
                    action_index = np.argmax(self.Qtable_1[state.digitized_state[0], state.digitized_state[1],
                                                            state.digitized_state[2], state.digitized_state[3], :] +
                                             self.Qtable_2[state.digitized_state[0], state.digitized_state[1],
                                                            state.digitized_state[2], state.digitized_state[3], :])
                ...
                if random_number < 0.5:
                    best_next_action_index = np.argmax(self.Qtable_1[next_state.digitized_state[0], next_state.digitized_state[1],
                                                                        next_state.digitized_state[2], next_state.digitized_state[3], :])
                    Q_target = (reward + self.discount_rate *
                                self.Qtable_2[next_state.digitized_state[0], next_state.digitized_state[1],
                                                next_state.digitized_state[2], next_state.digitized_state[3], best_next_action_index])

                    delta = learning_rate * (Q_target - self.Qtable_1[state.digitized_state[0], state.digitized_state[1],
                                                                    state.digitized_state[2], state.digitized_state[3], action_index])
                    self.Qtable_1[state.digitized_state[0], state.digitized_state[1],
                                    state.digitized_state[2], state.digitized_state[3], action_index] += delta
                else:
                    best_next_action_index = np.argmax(self.Qtable_2[next_state.digitized_state[0], next_state.digitized_state[1],
                                                                        next_state.digitized_state[2], next_state.digitized_state[3], :])
                    Q_target = (reward + self.discount_rate *
                                self.Qtable_1[next_state.digitized_state[0], next_state.digitized_state[1],
                                                next_state.digitized_state[2], next_state.digitized_state[3], best_next_action_index])

                    delta = learning_rate * (Q_target - self.Qtable_2[state.digitized_state[0], state.digitized_state[1],
                                                                        state.digitized_state[2], state.digitized_state[3], action_index])
                    self.Qtable_2[state.digitized_state[0], state.digitized_state[1],
                                    state.digitized_state[2], state.digitized_state[3], action_index] += delta
