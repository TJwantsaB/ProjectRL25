...
def run_experiments_concurrently(agent_class, agent_kwargs, n_runs):
    """
    Run multiple instances of the same agent concurrently using threads.
    :param agent_class: The class of the agent.
    ...
    with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        futures = [executor.submit(train_agent, agent_class, agent_kwargs) for _ in range(n_runs)]
...
if __name__ == "__main__":
    ...
    # Run experiments for Agents A, B, and C
    n_runs = 10  # Number of runs per agent
    agent_a_results = run_experiments_concurrently(QAgentDataCenter, agent_a_kwargs, n_runs)
    agent_b_results = run_experiments_concurrently(QAgentDataCenter, agent_b_kwargs, n_runs)
    agent_c_results = run_experiments_concurrently(QAgentDataCenter, agent_c_kwargs, n_runs)

    # Group results for plotting
    groups = {
        "Agent A": agent_a_results,
        "Agent B": agent_b_results,
        "Agent C": agent_c_results,
    }
